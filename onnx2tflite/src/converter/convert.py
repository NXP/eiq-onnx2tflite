...
def _assert_supported_opset(model: onnx_model.ModelProto, conversion_context: ConversionContext):
    ...
    onnx_opset_version = opset_map.get("", None)
...
def build_conversion_context(
    ...
    """
    description = "doc:'" + onnx_model.doc_string + f"' domain:'" + onnx_model.domain \
...
def convert_model(
        source: typing.Union[str, onnx.ModelProto],
        conversion_config: ConversionConfig = ConversionConfig(),
) -> bytearray:
                ...
                if conversion_config.skip_shape_inference:
                    try:
                        ...
                        logger.e(logger.Code.SHAPE_INFERENCE_ERROR,
                                 f"ONNX model's shapes not completely defined: {str(e)}")
                else:
