...
from onnx2tflite.src.onnx_parser.builtin_attributes import arg_min_attributes
...
class ArgMinConverter(NodeConverter):
    ...
    def convert(self, node: onnx_model.NodeProto, t_op: tflite_model.Operator) -> list[tflite_model.Operator]:
        ...
        if x.tensor_format.is_channels_last():
            ...
            # If it refers to `batch` or `channels`, a `Transpose` operator must be added before and after the `ArgMin`.
            ...
            if normalized_axis in [0, 1]:
                ...
                # The output of the `Transpose` is now `channels last`, but the output of the `ArgMin` is
