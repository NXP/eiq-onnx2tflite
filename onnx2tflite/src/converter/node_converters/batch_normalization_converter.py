...
class BatchNormalizationConverter(NodeConverter):
    ...
    def _convert_batch_normalization_with_static_operands(
        self, attrs: BatchNormalization, t_op: tflite_model.Operator
        ...
        if rank == 1:
            ...
            logger.e(
                ...
                "Conversion of ONNX BatchNormalization with a 1D main input is not "
    ...
    def _convert_batch_normalization_with_dynamic_operands(
        self, attrs: BatchNormalization, t_op: tflite_model.Operator
    ) -> list[tflite_model.Operator]:
    ...
    def convert(self, node: onnx_model.NodeProto, t_op: tflite_model.Operator) -> list[tflite_model.Operator]:
        ...
        if len(t_op.tmp_outputs) != 1:
            ...
            logger.e(
                logger.Code.INVALID_ONNX_MODEL,
                f"ONNX BatchNormalization has '{len(t_op.tmp_outputs)}' outputs. "
        ...
        if attrs.spatial == 0:
            ...
            if any(operand.rank != expected_operand_rank for operand in t_op.tmp_inputs[1:]):
                ...
                if any(operand.rank != 1 for operand in t_op.tmp_inputs[1:]):
                    ...
                    logger.w(
                        "ONNX BatchNormalization with 'spatial=0' should have operands with "
