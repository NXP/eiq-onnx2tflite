...
class QLinearConcatConverter(NodeConverter):
    ...
    def convert(self, node: onnx_model.NodeProto, t_op: tflite_model.Operator) -> list[tflite_model.Operator]:
        ...
        if len(set(io_types)) != 1:
            logger.e(
                logger.Code.INVALID_TYPE, "ONNX QLinearConcat at least one input type doesn't match with output type!"
            )
        ...
        for idx in range(2, len(t_op.tmp_inputs), 3):
            ...
            if is_int8_input and (not output_scale_match or not output_zero_point_match):
                output_scale_list, output_zero_point_list = quantization_params_to_lists(
                    output_scale, output_zero_point
                )

                if tensor_has_data(input_tensor):
                    set_quantization_parameters_to_tensor(input_tensor, input_scale, input_zero_point)
                    input_tensor = re_quantize_static_tensor(
                        self.builder, input_tensor, TensorType.INT8, output_scale_list, output_zero_point_list
                    )
        ...
        concatenation_op = tflite_model.Operator(builtin_options=tflite_concatenation_options.Concatenation(axis))
