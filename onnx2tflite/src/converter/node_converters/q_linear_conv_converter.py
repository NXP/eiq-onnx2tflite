...
from onnx2tflite.src.converter.quantization_utils import calculate_uint_to_int_re_quantization_zero_point, \
    get_symmetric_zero_point_for_type, re_quantize_static_tensor, set_quantization_parameters_to_tensor
...
class QLinearConvConverter(NodeConverter):
    ...
    def _get_bias_tensor_idx(self, attrs: ConvAttributes):
        if isinstance(attrs, q_linear_conv_attributes.QLinearConv):
            return 8
        elif isinstance(attrs, conv_attributes.Conv):
            return 2
        else:
    ...
    def _convert_2d_q_linear_conv(self, attrs: ConvAttributes, t_op: tflite_model.Operator) -> [tflite_model.Operator]:
        ...
        elif conv_utils.group_conv_convertible_into_multiple_convolutions(attrs, t_op):
