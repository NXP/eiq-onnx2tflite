...
def re_compute_q_params_for_type(tensor: tflite_model.Tensor, new_type: TensorType,
    ...
    :param symmetric: If True, returned zero_point will be symmetric for a given type.
    :return: Tuple with scale and zero_point values as lists.
    """
    if tensor.quantization is None:
        logger.e(logger.Code.INTERNAL_ERROR, f"Attempt to re-compute q-params for non-quantized tensor {tensor.name}.")

    scale = tensor.quantization.scale.vector
...
def re_quantize_static_tensor(
    ...
    if tensor_data.dtype == np.uint8 and new_dtype == np.int8:  # INT8 -> UINT8
        ...
        # ----- Universal pre channel re-quantization code (in case it's ever needed) -----
        # all_slices = []
        # for i in range(tensor_data.shape[axis]):
        #     tensor_slice = np.asarray(tensor_data.take(i, axis), np.int64)
        #     tensor_slice = np.asarray((tensor_slice - zero_point[i]) * scale[i], np.float64)
