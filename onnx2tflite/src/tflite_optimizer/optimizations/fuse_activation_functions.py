...
from onnx2tflite.src.tflite_optimizer.graph_utils import operator_is_type
from onnx2tflite.src.tflite_optimizer.operator_rules import NoFusedActivationFunction
...
from onnx2tflite.src.tflite_optimizer.tensor_rules import TensorHasOneConsumer
...
class FuseActivationFunctions(BaseOptimization):
    ...
    def __call__(self) -> bool:
        matcher = PatternMatcher(
            self._builder,
            [
                Op(self.ops_with_fused_activation_function, ["x"], ["x1"], [NoFusedActivationFunction()]),
        ...
        for [leading_op, act_fun_op], tensor_map, _, _ in matcher.match_patterns():
            ...
            logger.internal_assert(
                builtin_leading_op in self.supported_activations_for_op,
                f"FuseActivationFunctions: supported activations for operator `{builtin_leading_op}`are not known.",
            )
