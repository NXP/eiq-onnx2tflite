...
from onnx2tflite.src.tflite_optimizer.tensor_rules import TensorIsNotModelOutput, TensorsHaveSameQuantization
...
class PruneQuantizeOperators(BaseOptimization):
    ...
    def _is_quantization_recasting_from_float(self, quantize_input: tflite_model.Tensor,
        ...
        if next_op_input.type == TensorType.UINT8 and next_op_output.type == TensorType.INT8:
            return np.equal(input_zp, np.array(output_zp) + 128)
