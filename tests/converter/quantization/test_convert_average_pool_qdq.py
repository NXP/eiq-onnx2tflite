...
def qdq_model_average_pool():
    input_shape = [5, 3, 4, 6]

    graph = onnx.helper.make_graph(
        [onnx.helper.make_node("AveragePool", ["x"], ["output"], kernel_shape=[3, 3],
                               auto_pad="SAME_UPPER")],
        'AveragePool test',
...
def test_convert_average_pool_qdq(qdq_model_average_pool, intermediate_tflite_model_provider):
    input_shape = [5, 3, 4, 6]

    input_data = np.random.random(np.prod(input_shape)).reshape(input_shape).astype(np.float32)

    executors.convert_run_compare(qdq_model_average_pool, input_data, atol=0.04)
