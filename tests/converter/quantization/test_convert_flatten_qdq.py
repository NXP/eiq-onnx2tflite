...
def qdq_model_with_flatten():
    input_shape = [5, 1, 3, 1]

    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Flatten', ['input'], ['output'])],
        'flatten test',
...
def test_convert_flatten_qdq(qdq_model_with_flatten, intermediate_tflite_model_provider):
    input_shape = [5, 1, 3, 1]

    input_data = np.random.random(np.prod(input_shape)).reshape(input_shape).astype(np.float32)

    executors.convert_run_compare(qdq_model_with_flatten, input_data)
