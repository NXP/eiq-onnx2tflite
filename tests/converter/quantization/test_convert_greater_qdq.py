...
from onnx2tflite.src.tflite_generator.builtin_options import greater_options, quantize_options
from tests import executors


@pytest.fixture(scope="module")
def single_greater_node_qdq_model():
    quantizer = QDQQuantizer()
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node("Greater", ["x", "y"], ["output"])],
        'Greater test',
        [
...
@pytest.fixture(scope="module")
def single_greater_node_qdq_model__static_y():
    quantizer = QDQQuantizer()
...
def test_convert_greater_qdq(single_greater_node_qdq_model, intermediate_tflite_model_provider):
    x_shape = [2, 5]
    y_shape = [2, 5]
    ...
    executors.convert_run_compare(single_greater_node_qdq_model, input_data)
    ...
    assert isinstance(ops[2].builtin_options, greater_options.Greater)


def test_convert_greater_qdq__static_y(single_greater_node_qdq_model__static_y, intermediate_tflite_model_provider):
    x_shape = [2, 5]

    np.random.seed(23)
    input_data = np.random.random(x_shape).astype(np.float32)

    executors.convert_run_compare(single_greater_node_qdq_model__static_y, input_data)

    ops = intermediate_tflite_model_provider.get_operators()
    assert len(ops) == 2
    assert isinstance(ops[0].builtin_options, quantize_options.Quantize)
    assert isinstance(ops[1].builtin_options, greater_options.Greater)
