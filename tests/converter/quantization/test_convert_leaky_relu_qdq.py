...
from onnx2tflite.src.tflite_generator.builtin_options import dequantize_options, leaky_relu_options, quantize_options
from tests import executors


@pytest.fixture(scope="module")
def qdq_leaky_relu_model():
    input_shape = [4, 6]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node("LeakyRelu", ["x"], ["output"])
        ],
        'Softmax test',
...
def test_convert_leaky_relu_qdq(qdq_leaky_relu_model, intermediate_tflite_model_provider):
    input_shape = [4, 6]

    input_data = np.random.random(np.prod(input_shape)).reshape(input_shape).astype(np.float32)

    executors.convert_run_compare(qdq_leaky_relu_model, input_data)

    ops = intermediate_tflite_model_provider.get_operators()
    assert len(ops) == 3
    assert isinstance(ops[0].builtin_options, quantize_options.Quantize)
    assert isinstance(ops[1].builtin_options, leaky_relu_options.LeakyRelu)
