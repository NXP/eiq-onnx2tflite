...
def test_convert_min_qdq(intermediate_tflite_model_provider):
    np.random.seed(42)
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node("Min", ["x", "y"], ["output"])],
        'Min test',
        [
    ...
    executors.convert_run_compare(quantized_model, input_data, atol=0.0039)
    ...
    intermediate_tflite_model_provider.assert_converted_model_has_operators([
        ...
        BuiltinOperator.QUANTIZE, BuiltinOperator.MINIMUM, BuiltinOperator.DEQUANTIZE
    ])


def test_convert_minimum_qdq__static_y(intermediate_tflite_model_provider):
    x_shape = [2, 5]
    np.random.seed(42)
...
def test_convert_min_qdq__op_removed(intermediate_tflite_model_provider):
    np.random.seed(42)
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node("Softmax", ["x"], ["y"]),
            onnx.helper.make_node("Min", ["y"], ["z"]),
