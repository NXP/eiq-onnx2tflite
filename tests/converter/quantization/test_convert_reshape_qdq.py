...
def qdq_model_with_reshape():
    input_shape = [5, 3, 16, 16]
    output_shape = [5, 48, 16]
...
def test_convert_reshape_qdq(qdq_model_with_reshape, intermediate_tflite_model_provider):
    input_shape = [5, 3, 16, 16]

    input_data = np.random.random(np.prod(input_shape)).reshape(input_shape).astype(np.float32)

    executors.convert_run_compare(qdq_model_with_reshape, input_data, atol=0.004)
