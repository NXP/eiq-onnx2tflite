...
def qdq_model_sigmoid():
    input_shape = [4, 6]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node("Sigmoid", ["x"], ["output"])
        ],
        'Softmax test',
...
def test_convert_sigmoid_qdq(qdq_model_sigmoid, intermediate_tflite_model_provider):
    input_shape = [4, 6]

    input_data = np.random.random(np.prod(input_shape)).reshape(input_shape).astype(np.float32)

    executors.convert_run_compare(qdq_model_sigmoid, input_data)
...
def test_convert_sigmoid_qdq__non_matching_q_params(quant_type, zero_point):
    ...
    graph = onnx.helper.make_graph(
        [
            ...
            onnx.helper.make_node("Sigmoid", ["x_dequant"], ["y"]),
