...
from onnx2tflite.src.tflite_generator.builtin_options import dequantize_options, quantize_options, sub_options
from tests import executors


@pytest.fixture(scope="module")
def single_sub_node_qdq_model():
...
@pytest.fixture(scope="module")
def single_sub_node_qdq_model__static_y():
    quantizer = QDQQuantizer()
...
def test_convert_sub_qdq(single_sub_node_qdq_model, intermediate_tflite_model_provider):
    x_shape = [2, 5]
    y_shape = [2, 5]
    ...
    executors.convert_run_compare(single_sub_node_qdq_model, input_data)
    ...
    assert isinstance(ops[2].builtin_options, sub_options.Sub)
    assert isinstance(ops[3].builtin_options, dequantize_options.Dequantize)


def test_convert_sub_qdq__static_y(single_sub_node_qdq_model__static_y, intermediate_tflite_model_provider):
    x_shape = [2, 5]

    input_data = np.random.random(np.prod(x_shape)).reshape(x_shape).astype(np.float32)

    executors.convert_run_compare(single_sub_node_qdq_model__static_y, input_data)

    ops = intermediate_tflite_model_provider.get_operators()
    assert len(ops) == 3
    assert isinstance(ops[0].builtin_options, quantize_options.Quantize)
    assert isinstance(ops[1].builtin_options, sub_options.Sub)
