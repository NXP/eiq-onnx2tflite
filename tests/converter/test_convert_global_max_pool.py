...
@pytest.mark.parametrize(
    "input_shape",
    [
        pytest.param([4, 6, 8], id='3D'),
        pytest.param([2, 4, 6, 8], id='4D'),
        pytest.param([2, 3, 4, 5, 6], id='5D')
    ])
def test_convert_global_max_pool(input_shape: list[int]):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('GlobalMaxPool', ['x'], ['y'])],
        'GlobalMaxPool test',
...
def test_convert_global_max_pool_with_unsupported_type():
    # Double
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('GlobalMaxPool', ['x'], ['y'])],
        'GlobalMaxPool test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.DOUBLE, input_shape)],
...
def test_convert_global_max_pool__quantized(type_: TensorProto.DataType):
    shape = [2, 4, 6, 8]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('GlobalMaxPool', ['x1'], ['y'])
        ],
