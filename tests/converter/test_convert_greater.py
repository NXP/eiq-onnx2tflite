...
def test_convert_greater__types(type_: TensorProto.DataType):
    x_shape = [100]
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Greater', ['x', 'y'], ['o'])],
        'Greater test',
        [
...
def test_convert_greater__unsupported_type():
...
def test_convert_greater__different_input_types():
    input_shape = [100]
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Greater', ['x', 'y'], ['o'])],
        'Greater test',
...
def test_convert_greater__shape_broadcasting(x_shape, y_shape):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Greater', ['x', 'y'], ['o'])],
        'Greater test',
        [
...
def test_convert_greater__channels_first_shape_broadcasting(x_shape, y_shape):
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['o1'], kernel_shape=[1]),
            onnx.helper.make_node('Greater', ['o1', 'y'], ['o'])
        ],
        'Greater test',
        [
            onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape),
