...
def test_convert_min__1_input_skipped(shape: list[int], intermediate_tflite_model_provider):
    np.random.seed(42)
    data = np.random.random(shape).astype(np.float32)

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('Min', ['x'], ['x1']),
...
def test_convert_min__1_input_not_skipped(shape: list[int], intermediate_tflite_model_provider):
    np.random.seed(42)
    data = np.random.random(shape).astype(np.float32)

    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Min', ['x'], ['y'])],
        'Min test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
    )
...
def test_convert_min__2_inputs(shape1: list[int], shape2: list[int]):
    np.random.seed(42)
    data = {
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Min', ['x1', 'x2'], ['y'])],
        'Min test',
        [
...
def test_convert_min__3_inputs():
    shape = [42]
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Min', ['x1', 'x2', 'x3'], ['y'])],
        'Min test',
        [
...
def test_convert_min__types(data_type: TensorProto.DataType):
    shape = [42]
...
def test_convert_min__unsupported_type():
...
def test_convert_min__quantized(type_: TensorProto.DataType):
    shape = [3, 14, 15]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('Min', ['x1', 'x1'], ['y'])
        ],
