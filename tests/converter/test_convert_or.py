...
def test_convert_or__shape_broadcasting__formatless(a_shape: list[int], b_shape: list[int]):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Or', ['a', 'b'], ['y'])],
        'Or test',
        [
...
def test_convert_or__shape_broadcasting__channels_first(a_shape: list[int], b_shape: list[int]):
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('Or', ['a', 'b'], ['x1']),
...
def test_convert_or__unsupported_type():
    type_ = TensorProto.DOUBLE

    shape = [256]
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Or', ['x', 'x'], ['y'])],
        'Or test',
