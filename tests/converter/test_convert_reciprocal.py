...
def test_convert_reciprocal(input_shape: List[int]):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Reciprocal', ['input'], ['output'])],
        'Reciprocal test',
...
def test_convert_reciprocal__channels_first(input_shape: List[int]):
    kernel_shape = [1] * (len(input_shape) - 2)

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node("Reciprocal", ["x"], ["reciprocal_out"]),
            onnx.helper.make_node("MaxPool", ["reciprocal_out"], ["output"], kernel_shape=kernel_shape),
        ],
        'Reciprocal test',
...
def test_convert_reciprocal_unsupported_input_type():
    input_shape = [5, 48, 16]
...
def test_convert_reciprocal__quantized(type_: TensorProto.DataType):
    shape = [3, 14, 15]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('Reciprocal', ['x1'], ['y'])
        ],
