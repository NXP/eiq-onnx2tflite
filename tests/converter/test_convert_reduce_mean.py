...
def test_convert_reduce_mean__v18(axes: list[int]):
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceMean', ['x', 'axes'], ['y'])],
        'ReduceMean test',
...
def test_convert_reduce_mean__v18_channels_first_static_axes(axes: list[int]):
    x_shape = [5, 10, 15, 20]
    ...
    graph = onnx.helper.make_graph(
        [
            ...
            onnx.helper.make_node('ReduceMean', ['y1', 'axes'], ['y'])
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__channels_first_input_and_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceMean', ['y1', 'axes'], ['y2'], keepdims=0),
...
@pytest.mark.parametrize("axes", [[0], [1], [2], [3]], ids=lambda x: f'axes={x}')
def test_convert_reduce_mean__channels_first_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('ReduceMean', ['x', 'axes'], ['y2'], keepdims=0),
            onnx.helper.make_node('MaxPool', ['y2'], ['y'], kernel_shape=[1]),
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
...
def test_convert_reduce_mean__v18_channels_first__keepdims_false__different_axes(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceMean', ['y1', 'axes'], ['y'], keepdims=0)
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__v18_dynamic_axes():
    x_shape = [5, 10, 15, 20]
...
def test_convert_reduce_mean__v18_inferred_dynamic_axes(indices: list[int]):
    x_shape = [1, 2, 3, 20]
...
def test_convert_reduce_mean__v18_channels_first_inferred_dynamic_axes(indices: list[int]):
    x_shape = [1, 2, 3, 20]
    ...
    graph = onnx.helper.make_graph(
        [
            ...
            onnx.helper.make_node('Shape', ['x'], ['shape']),
            onnx.helper.make_node('Gather', ['shape', 'indices'], ['axes']),
            onnx.helper.make_node('ReduceMean', ['x1', 'axes'], ['y'])
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__v18_dynamic_axes__cast_operator():
...
def test_convert_reduce_mean__v18_default_axes():
    x_shape = [5, 10, 15]
...
def test_convert_reduce_mean__v13(axes: list[int]):
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceMean', ['x'], ['y'], axes=axes)],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
    )
...
def test_convert_reduce_mean__v13_channels_first_input(axes: list[int]):
    x_shape = [5, 10, 15, 20]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['x1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceMean', ['x1'], ['y'], axes=axes)
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__v13_channels_first_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('ReduceMean', ['x'], ['y1'], keepdims=0, axes=axes),
...
@pytest.mark.parametrize("axes", [[0], [1], [2], [-1], [-2]], ids=lambda x: f'axes={x}')
def test_convert_reduce_mean__v13_channels_first_input_and_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceMean', ['y1'], ['y2'], keepdims=0, axes=axes),
            onnx.helper.make_node('MaxPool', ['y2'], ['y'], kernel_shape=[1]),
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
...
def test_convert_reduce_mean__v13_channels_first_different_keepdims(keepdims):
    x_shape = [1, 1, 2, 3]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['x1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceMean', ['x1'], ['y'], axes=[1], keepdims=keepdims)
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__v13_channels_first__keepdims_false__different_axes(axes):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['x1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceMean', ['x1'], ['y'], axes=axes, keepdims=0)
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__v13_default_axes():
    x_shape = [5, 10, 15]
...
def test_convert_reduce_mean__noop_with_empty_axes(intermediate_tflite_model_provider):
    x_shape = [5, 10, 15]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('ReduceMean', ['x'], ['y'], noop_with_empty_axes=1),
        ],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__keepdims_true():
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceMean', ['x', 'axes'], ['y'], keepdims=True)],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_mean__keepdims_false():
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceMean', ['x', 'axes'], ['y'], keepdims=False)],
        'ReduceMean test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
        [onnx.helper.make_tensor('axes', TensorProto.INT64, [2], [0, 2])]
...
def test_convert_reduce_mean__types(type_: TensorProto.DataType):
    shape = [42]
...
def test_convert_reduce_mean__unsupported_type():
...
def test_convert_reduce_mean__quantized(type_: TensorProto.DataType):
    shape = [3, 14, 15]
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('ReduceMean', ['x1'], ['x2']),
