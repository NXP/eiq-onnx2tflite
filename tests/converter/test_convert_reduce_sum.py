...
def test_convert_reduce_sum__v13(axes: list[int]):
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceSum', ['x', 'axes'], ['y'])],
        'ReduceSum test',
...
def test_convert_reduce_sum__v13_channels_first_static_axes(axes: list[int]):
    x_shape = [5, 10, 15, 20]

    np.random.seed(42)

    x_data = np.arange(np.prod(x_shape)).reshape(x_shape).astype(np.float32) - 100.

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['y1', 'axes'], ['y'])
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__channels_first_input_and_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['y1', 'axes'], ['y2'], keepdims=0),
...
@pytest.mark.parametrize("axes", [[0], [1], [2], [3]], ids=lambda x: f'axes={x}')
def test_convert_reduce_sum__channels_first_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('ReduceSum', ['x', 'axes'], ['y2'], keepdims=0),
            onnx.helper.make_node('MaxPool', ['y2'], ['y'], kernel_shape=[1]),
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
...
def test_convert_reduce_sum__v13_channels_first__keepdims_false__different_axes(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['y1', 'axes'], ['y'], keepdims=0)
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__v13_dynamic_axes():
    x_shape = [5, 10, 15, 20]
...
def test_convert_reduce_sum__v13_inferred_dynamic_axes(indices: list[int]):
    x_shape = [1, 2, 3, 20]
...
def test_convert_reduce_sum__v13_dynamic_axes__cast_operator():
...
def test_convert_reduce_sum__v13_channels_first_inferred_dynamic_axes(indices: list[int]):
    x_shape = [1, 2, 3, 20]
...
def test_convert_reduce_sum__v13_default_axes():
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceSum', ['x', ''], ['y'])],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
    )
...
def test_convert_reduce_sum__v11(axes: list[int]):
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceSum', ['x'], ['y'], axes=axes)],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
    )
...
def test_convert_reduce_sum__v11_channels_first_input(axes: list[int]):
    x_shape = [5, 10, 15, 20]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['x1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['x1'], ['y'], axes=axes)
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__v11_channels_first_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('ReduceSum', ['x'], ['y1'], keepdims=0, axes=axes),
...
@pytest.mark.parametrize("axes", [[0], [1], [2], [-1], [-2]], ids=lambda x: f'axes={x}')
def test_convert_reduce_sum__v11_channels_first_input_and_output(axes: list[int]):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['y1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['y1'], ['y2'], keepdims=0, axes=axes),
            onnx.helper.make_node('MaxPool', ['y2'], ['y'], kernel_shape=[1]),
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
...
def test_convert_reduce_sum__v11_channels_first_different_keepdims(keepdims):
    x_shape = [1, 1, 2, 3]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['x1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['x1'], ['y'], axes=[1], keepdims=keepdims)
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__v11_channels_first__keepdims_false__different_axes(axes):
    x_shape = [2, 3, 4, 5]
    ...
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('MaxPool', ['x'], ['x1'], kernel_shape=[1, 1]),
            onnx.helper.make_node('ReduceSum', ['x1'], ['y'], axes=axes, keepdims=0)
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__v11_default_axes():
    x_shape = [5, 10, 15]
...
def test_convert_reduce_sum__noop_with_empty_axes(intermediate_tflite_model_provider):
    x_shape = [5, 10, 15]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('ReduceSum', ['x'], ['y'], noop_with_empty_axes=1),
        ],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__keepdims_true():
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceSum', ['x', 'axes'], ['y'], keepdims=True)],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
...
def test_convert_reduce_sum__keepdims_false():
    x_shape = [5, 10, 15]
    ...
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('ReduceSum', ['x', 'axes'], ['y'], keepdims=False)],
        'ReduceSum test',
        [onnx.helper.make_tensor_value_info('x', TensorProto.FLOAT, x_shape)],
        [onnx.helper.make_tensor_value_info('y', TensorProto.FLOAT, ())],
        [onnx.helper.make_tensor('axes', TensorProto.INT64, [2], [0, 2])]
