...
def test_convert_sigmoid(input_shape: List[int]):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Sigmoid', ['x'], ['y'])],
        'Sigmoid test',
...
def test_convert_sigmoid_unsupported_input_type():
    input_shape = [5, 48, 16]
...
def test_convert_sigmoid_invalid_input_type():
    input_shape = [5, 48, 16]
...
def test_convert_sigmoid__quantized(type_: TensorProto.DataType):
    shape = [3, 14, 15]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('Sigmoid', ['x1'], ['y'])
        ],
