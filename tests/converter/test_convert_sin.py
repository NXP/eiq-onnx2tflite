...
def test_convert_sin__shapes(shape: list[int]):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Sin', ['x'], ['y'])],
        'Sin test',
...
def test_convert_sin__unsupported_type():
    shape = [42]
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Sin', ['x'], ['y'])],
        'Sin test',
...
def test_convert_sin__quantized(type_: TensorProto.DataType):
    shape = [3, 14, 15]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('Sin', ['x1'], ['y'])
        ],
