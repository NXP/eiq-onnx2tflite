...
def test_convert_softmax(input_shape, axis, opset):
...
@pytest.mark.parametrize("opset", [11, 13], ids=(lambda x: f"opset={x}"))
def test_convert_softmax_after_conv(axis: int, opset: int):
    ...
    node_softmax = onnx.helper.make_node('Softmax', inputs=['y'], outputs=['z'], axis=axis)

    x_shape = (5, 3, 16, 16)
    w_shape = (3, 3, 3, 3)

    graph = onnx.helper.make_graph(
        [node_conv, node_softmax],
        'conv+softmax',
        [
    ...
    original_model = onnx.helper.make_model(
        graph,
        producer_name="onnx-conv-softmax",
        opset_imports=[onnx.helper.make_opsetid("", opset)])
    onnx.checker.check_model(original_model)

    input_data = {
        0: np.linspace(0, 1, math.prod(x_shape)).reshape(x_shape).astype(np.float32),
        1: np.linspace(0, 1, math.prod(w_shape)).reshape(w_shape).astype(np.float32)
    }
...
@pytest.mark.parametrize("opset", [11, 13], ids=(lambda x: f"opset={x}"))
def test_convert_softmax__default_axis(opset):
    input_shape = (2, 3, 4, 5)
    node = onnx.helper.make_node("Softmax", ["x"], ["y"])
...
def test_convert_softmax__invalid_type():
...
def test_convert_softmax__quantized(type_: TensorProto.DataType):
    shape = [5, 10]

    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('QuantizeLinear', ['x', 's', 'zp'], ['x1']),
            onnx.helper.make_node('Softmax', ['x1'], ['y'])
        ],
