...
def test_convert_xor__shape_broadcasting__format_less(a_shape: list[int], b_shape: list[int]):
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Xor', ['a', 'b'], ['y'])],
        'Xor test',
        [
...
def test_convert_xor__shape_broadcasting__channels_first(a_shape: list[int], b_shape: list[int]):
    graph = onnx.helper.make_graph(
        [
            onnx.helper.make_node('Xor', ['a', 'b'], ['x1']),
...
def test_convert_xor__unsupported_type():
    type_ = TensorProto.DOUBLE

    shape = [256]
    graph = onnx.helper.make_graph(
        [onnx.helper.make_node('Xor', ['x', 'x'], ['y'])],
        'Xor test',
